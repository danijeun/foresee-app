"""
XGBoost Agent for Foresee App
Trains XGBoost models and outputs key insights to Snowflake
"""
import sys
...
class XGBoostAgent:
    """
    Agent that trains XGBoost models and generates insights
    """
    
    def __init__(self):
        """
        Initialize the XGBoost Agent
        """
    ...
    def train_and_evaluate(
        ...
        """
        Train XGBoost model and save insights to Snowflake
        ...
        try:
            ...
            if target_info['problem_type'] and 'regression' in target_info['problem_type'].lower():
                print(f"   ⚠️  WARNING: Target appears to be for regression, but XGBoost is configured for classification.")
            ...
            evaluation = self._evaluate_model(
                X_train, X_test, y_train, y_test, feature_names, n_classes
            )
            ...
            print(f"\n🗄️  Step 8: Creating XGBOOST_SUMMARY table...")
    ...
    def _preprocess_data(
        ...
        # Drop date/time columns (not suitable for XGBoost without feature engineering)
        datetime_cols = X.select_dtypes(include=['datetime64']).columns.tolist()
        if datetime_cols:
            X = X.drop(columns=datetime_cols)
            preprocessing_report['dropped_columns'].extend([f"{col} (datetime)" for col in datetime_cols])
        
        # Handle categorical features - XGBoost handles them well with encoding
        ...
        for col in categorical_cols:
            ...
            if unique_count > 200:  # XGBoost can handle more categories
                X = X.drop(columns=[col])
                preprocessing_report['dropped_columns'].append(
        ...
        # Handle missing values in numeric columns - XGBoost handles NaN natively but we'll impute
...
XGBoost ensemble with {n_estimators} trees at max depth {max_depth}.
"""
...
N_estimators: {n_estimators}, Max depth: {max_depth}, Learning rate: {learning_rate}.
...
"""
        ...
        if evaluation['test_accuracy'] < 0.7:
            recommendations.append("Low accuracy. Try increasing n_estimators or tuning hyperparameters.")
        
        if accuracy_diff > 0.1:
            recommendations.append("Model may be overfitting. Try reducing max_depth or increasing subsample/colsample_bytree.")
        
        if accuracy_diff > 0.15:
            recommendations.append("Severe overfitting. Consider early stopping or reducing model complexity.")
        ...
        if evaluation['test_accuracy'] > 0.95 and accuracy_diff < 0.05:
            recommendations.append("Excellent performance! Model generalizes well and is ready for deployment.")
    ...
    def _create_summary_table(self, uploader, schema_name: str):
        """
        Create XGBOOST_SUMMARY table if it doesn't exist
        """
        create_table_sql = f"""
        CREATE TABLE IF NOT EXISTS {schema_name}.XGBOOST_SUMMARY (
        ...
        print(f"   ✓ Table {schema_name}.XGBOOST_SUMMARY ready")
    ...
    def _save_insights_to_snowflake(
        ...
        """
        Save insights to XGBOOST_SUMMARY table
        """
        analysis_id = f"XGB_{workflow_id}_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
        ...
        insert_sql = f"""
        INSERT INTO {schema_name}.XGBOOST_SUMMARY (
            analysis_id,
            workflow_id,
            table_name,
            target_variable,
        ...
        uploader.cursor.execute(insert_sql, (
            ...
            insights_clean['n_estimators'],
            insights_clean['max_depth'],
            insights_clean['learning_rate'],
            insights_clean['subsample'],
            insights_clean['colsample_bytree'],
        ...
        print(f"   ✓ Location: {schema_name}.XGBOOST_SUMMARY")
...
if __name__ == "__main__":
    """
    Example usage of the XGBoost Agent
    """
    import traceback
