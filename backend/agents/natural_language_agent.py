...
"""
    ...
    def _generate_introduction_text(self, metadata: Dict[str, Any], sections: Dict[str, Any]) -> str:
        """
        Generate a comprehensive introduction text for the PDF report
        """
        try:
            dataset = sections['1_dataset']
            data_analysis = sections['2_data_analysis']
            models = sections['3_models_performance']
            best_model = sections['4_best_model']
            
            # Build introduction paragraph
            intro = f"This report presents a comprehensive machine learning analysis conducted on the <b>{metadata['table_name']}</b> dataset. "
            intro += f"The dataset contains <b>{dataset['statistics']['Total Rows']:,} records</b> with <b>{dataset['statistics']['Total Columns']} features</b>, "
            intro += f"collected and analyzed to predict the target variable <b>{data_analysis['target_variable']}</b>. "
            
            # Add problem type context
            problem_type = data_analysis['analysis_type'].lower()
            if problem_type == 'classification':
                intro += "This is a classification problem where the goal is to predict categorical outcomes based on input features. "
            elif problem_type == 'regression':
                intro += "This is a regression problem aimed at predicting continuous numerical values based on input features. "
            elif problem_type == 'supervised':
                intro += "This supervised learning analysis aims to build predictive models using labeled training data. "
            
            # Add models trained information
            num_models = len(models['metrics_comparison_table'])
            intro += f"To achieve optimal predictive performance, <b>{num_models} different machine learning algorithms</b> were trained and evaluated, including "
            
            model_names = [m['Model'] for m in models['metrics_comparison_table']]
            if len(model_names) == 1:
                intro += f"{model_names[0]}. "
            elif len(model_names) == 2:
                intro += f"{model_names[0]} and {model_names[1]}. "
            else:
                intro += ", ".join(model_names[:-1]) + f", and {model_names[-1]}. "
            
            # Add best model performance
            best_accuracy = best_model['performance_metrics']['accuracy']
            intro += f"The analysis identified <b>{best_model['model_type']}</b> as the top-performing model, achieving an impressive accuracy of <b>{best_accuracy:.2%}</b>. "
            
            # Add data quality insight
            if dataset['statistics'].get('Duplicate Rows'):
                dup_pct = (dataset['statistics']['Duplicate Rows'] / dataset['statistics']['Total Rows']) * 100
                if dup_pct > 5:
                    intro += f"The dataset quality analysis revealed {dup_pct:.1f}% duplicate records that may require attention. "
                else:
                    intro += "The dataset demonstrates good quality with minimal duplicate records. "
            
            # Add feature importance insight
            if best_model.get('top_features') and len(best_model['top_features']) > 0:
                top_feature = best_model['top_features'][0]
                intro += f"Feature importance analysis identified <b>{top_feature['feature']}</b> as the most influential predictor, "
                intro += f"contributing significantly to the model's predictive power. "
            
            # Add closing statement
            intro += "This analysis provides detailed insights into model performance, feature relationships, and actionable recommendations for deployment and continuous improvement."
            
            return intro
        except Exception as e:
            print(f"   ⚠️  Error generating introduction text: {e}")
            return "This report provides a comprehensive machine learning analysis with multiple models trained and evaluated for optimal predictive performance."
    ...
    def _generate_pdf_report(self, output: Dict[str, Any], json_path: str) -> Optional[str]:
        ...
        try:
            ...
            # ======================
            story.append(Paragraph("Introduction", styles["SectionHeader"]))
            story.append(Spacer(1, 4))
            story.append(Paragraph("─" * 90, styles["Subtle"]))
            story.append(Spacer(1, 8))
            
            # Generate comprehensive introduction
            intro_text = self._generate_introduction_text(metadata, sections)
            story.append(Paragraph(intro_text, styles["Justified"]))
            story.append(Spacer(1, 12))
            
            # Analysis Summary Box
            story.append(Paragraph("<b>Analysis Summary</b>", styles["SubHeader"]))
            story.append(Spacer(1, 6))
            
            summary_data = [
                ["Dataset Name", metadata['table_name']],
                ["Total Records", f"{sections['1_dataset']['statistics']['Total Rows']:,}"],
                ["Total Features", f"{sections['1_dataset']['statistics']['Total Columns']}"],
                ["Target Variable", sections['2_data_analysis']['target_variable']],
                ["Problem Type", sections['2_data_analysis']['analysis_type'].title()],
                ["Models Trained", str(len(sections['3_models_performance']['metrics_comparison_table']))],
                ["Best Model", sections['4_best_model']['model_type']],
                ["Best Accuracy", f"{sections['4_best_model']['performance_metrics']['accuracy']:.2%}"],
                ["Analysis Date", gen_date],
            ]
            ...
            # Objectives section
            story.append(Paragraph("<b>Analysis Objectives</b>", styles["SubHeader"]))
            story.append(Spacer(1, 4))
            objectives = [
                f"Predict <b>{sections['2_data_analysis']['target_variable']}</b> with maximum accuracy",
                "Compare multiple machine learning algorithms to identify the best performer",
                "Identify the most important features driving predictions",
                "Assess data quality and provide recommendations for improvement",
                "Generate actionable insights for model deployment and optimization"
            ]
            ...
            story.append(Spacer(1, 8))
            story.append(Paragraph("<i>This report provides a comprehensive analysis of your data and machine learning models, including performance metrics, feature importance analysis, and actionable recommendations.</i>", styles["Justified"]))
