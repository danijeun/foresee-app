"""
Logistic Regression Agent for Foresee App
Trains logistic regression models and outputs key insights to Snowflake
"""
import sys
...
class LogisticRegressionAgent:
    """
    Agent that trains logistic regression models and generates insights
    """
    
    def __init__(self):
        """
        Initialize the Logistic Regression Agent
        """
    ...
    def train_and_evaluate(
        ...
        """
        Train logistic regression model and save insights to Snowflake
        ...
        try:
            print("=" * 70)
            print("🤖 LOGISTIC REGRESSION AGENT")
            print("=" * 70)
            ...
            if target_info['problem_type'] and 'regression' in target_info['problem_type'].lower():
                print(f"   ⚠️  WARNING: Target appears to be for regression, but Logistic Regression is for classification.")
            ...
            print(f"\n🗄️  Step 8: Creating LOGISTIC_REGRESSION_SUMMARY table...")
    ...
    def _preprocess_data(
        ...
        # Drop date/time columns (not suitable for logistic regression)
        ...
        preprocessing_report['final_rows'] = len(X_scaled)
        preprocessing_report['final_features'] = len(feature_names)
        
        return X_scaled, y_array, feature_names, preprocessing_report
    ...
    def _get_feature_importance(self, feature_names: List[str]) -> List[Dict[str, Any]]:
        ...
        importance = sorted(importance, key=lambda x: x['abs_coefficient'], reverse=True)
...
"""
        ...
        top_features_summary = "Top 5 most important features:\n" + "\n".join([
            f"  {i+1}. {f['feature']}: {f['coefficient']:.4f}"
...
"""
        ...
        if evaluation['test_accuracy'] < 0.7:
            recommendations.append("Consider feature engineering or trying more complex models (Random Forest, XGBoost).")
        
        if accuracy_diff > 0.1:
            recommendations.append("Model may be overfitting. Try regularization or collect more data.")
    ...
    def _create_summary_table(self, uploader, schema_name: str):
        """
        Create LOGISTIC_REGRESSION_SUMMARY table if it doesn't exist
        """
        create_table_sql = f"""
        CREATE TABLE IF NOT EXISTS {schema_name}.LOGISTIC_REGRESSION_SUMMARY (
        ...
        print(f"   ✓ Table {schema_name}.LOGISTIC_REGRESSION_SUMMARY ready")
    ...
    def _save_insights_to_snowflake(
        ...
        """
        Save insights to LOGISTIC_REGRESSION_SUMMARY table
        """
        analysis_id = f"LR_{workflow_id}_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
        ...
        insert_sql = f"""
        INSERT INTO {schema_name}.LOGISTIC_REGRESSION_SUMMARY (
            analysis_id,
            workflow_id,
            table_name,
            target_variable,
        ...
        print(f"   ✓ Location: {schema_name}.LOGISTIC_REGRESSION_SUMMARY")
...
if __name__ == "__main__":
    """
    Example usage of the Logistic Regression Agent
    """
    import traceback
